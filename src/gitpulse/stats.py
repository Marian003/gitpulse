...
# Count language usage -- O(1) dict lookups
def compute_languages(repos: list[dict[str, Any]], top_n: int = 6) -> list[LanguageBreakdown]:
    ...
    for repo in repos:
        ...
        if lang is not None and lang != "":  # skip repos with no language set
...
def compute_streak(events: list[dict[str, Any]]) -> StreakInfo:
    ...
    for event in events:
        ...
        if d:
            push_dates.add(d)
    if not push_dates:  # user has no public push events -- streak is zero ??? return zeros
